        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    if names[c] != 'person':  # 'person' 클래스가 아니면 무시
                        continue  # 다음 객체로 넘어감
                    ##########
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        person_list.append((xyxy, crop_img))  ###추가###
                        
                        for person_xyxy, poseImg in person_list:
                            #Load the cropped image with label
                            #crop_img = cv2.imread(str(person_file))
                            # Paste back the cropped image to the original image
                            im0 = paste_back(im0, poseImg, person_xyxy)
...
def paste_back(im, crop_img, xyxy):
    ...
    if im[y1:y2, x1:x2].shape == resized_crop_img.shape:
        ...
        print(f"Image size mismatch: {im[y1:y2, x1:x2].shape} != {resized_crop_img.shape}")
