...
def gendata(data_path,
    ...
    # 클래스 정의
    normal_classes = [8, 9, 28, 12, 11, 27, 80]  # normal 클래스
    aggression_classes = [50]  # aggression 클래스
    tumbles_classes = [43]  # tumbles 클래스
    ...
    for filename in os.listdir(data_path):
        ...
        if action_class in normal_classes:
            label = 0  # normal
        elif action_class in aggression_classes:
            label = 1  # aggression
        elif action_class in tumbles_classes:
            label = 2  # tumbles
        else:
            continue  # 다른 클래스는 무시

        if benchmark == 'xview_bpsd':
            istraining = (camera_id in training_cameras)
        elif benchmark == 'xsub_bpsd':
...
if __name__ == '__main__':
    ...
    benchmark = ['xsub_bpsd', 'xview_bpsd']
    part = ['train', 'val']
